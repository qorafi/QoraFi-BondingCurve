...
![QoraFi Logo](https://placehold.co/600x300/1a202c/ffffff?text=QoraFi+Protocol)
...
Execution Time Formula: executeAfter = block.timestamp + EMERGENCY_TIMELOCK_DELAY
...
Multi-Observation TWAP: Instead of a simple TWAP, the oracle uses a weighted average of multiple price observations stored in a circular buffer. This makes it significantly more expensive and difficult to manipulate the price over the required time window.

Price Validation: Includes circuit breakers that validate any new price against the last known price and the last known market cap, reverting if the change is too drastic.

Liveness & Liquidity Checks: The oracle will not provide a price if the underlying liquidity in the PancakeSwap pool is too low or if the price data is stale.

Key Functions:

updateMarketCap(): The core function called by a keeper bot. It updates the TWAP observations and recalculates the qorafiPriceTwap and cachedMarketCap.

_getEnhancedTWAPPrice(): Calculates the time-weighted average price across multiple valid observations stored in the twapObservations array.

Formula: Price_TWAP=
fracsum_i=1 
n
 (Price_itimesTimeElapsed_i)sum_i=1 
n
 TimeElapsed_i

Where Price_i is the calculated price for the period between observation i and i−1.

And TimeElapsed_i is the duration between observation i and i−1.

_calculateTwapPrice(priceCumulativeDiff, timeElapsed): This function contains the core mathematical formula for converting Uniswap V2's cumulative price into a human-readable price.

Formula (if QoraFi is token1):
Price=
fracPriceCumulativeDiffTimeElapsed
times
frac10 
Decimals_USDT
 2 
...
_validatePriceChange(oldPrice, newPrice): A circuit breaker that ensures the price does not change by more than a governance-defined percentage in a single update.

Formula: 
frac∣newPrice−oldPrice∣oldPrice
times10000
lemaxPriceChangeBPS

_validateMarketCapGrowth(oldCap, newCap): A circuit breaker that ensures the market cap does not grow by more than a governance-defined percentage in a single update.

Formula: 
fracnewCap−oldCapoldCap
times10000
lemaxMarketCapGrowthBPS
...
depositWithBNB(...) / depositWithToken(...): "Zap-in" functions that allow users to deposit with BNB or other whitelisted tokens, which are automatically swapped to USDT.
...
_swapAndSplit(...): Splits the user's deposit and calculates a secure minimum output for the swap using a dynamic slippage calculation.

Slippage Protection Formula: minOutput=expectedOutput
times
frac10000−maxSlippageBPS10000
...
handleStakeChange(...): Called by the ProofOfLiquidity vault to update a user's collateral value for reward calculations.

claimMyRewards(...): The user-facing function to claim accrued rewards, which triggers the minting of new QoraFi tokens.

getCurrentDailyRewardPercentageBPS(): A view function that calculates the current reward rate based on the quarterly decay schedule. The rate decreases every 90 days.
...
Approve USDT: The user first approves the BondingCurve contract to spend their USDT.

Deposit: The user calls the deposit function on the BondingCurve contract with the amount of USDT they wish to contribute.

Receive LP Tokens: The BondingCurve contract automatically swaps half of the USDT for QoraFi, pairs it with the remaining USDT, adds the liquidity to PancakeSwap, and sends the resulting LP tokens to the user.

Stake LP Tokens: The user then calls the stake function on the ProofOfLiquidity contract to stake their newly acquired LP tokens and start earning rewards.
