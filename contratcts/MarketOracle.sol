...
/**
 * @title MarketOracle (Enhanced & Fixed for New Token)
 * @notice A secure, upgradeable oracle for the BondingCurve with enhanced protection against single-LP manipulation.
...
contract MarketOracle is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function setUpdateInterval(uint256 _newInterval) external onlyRole(GOVERNANCE_ROLE) {
        ...
        priceValidation.minTimeBetweenUpdates = _newInterval;
    }
    ...
    function setMinimumUsdtLiquidity(uint256 _minUsdtLiquidity) external onlyRole(GOVERNANCE_ROLE) {
        require(_minUsdtLiquidity >= 1000 * (10 ** usdtDecimals), "Minimum too low"); // At least $1k
        minimumUsdtLiquidity = _minUsdtLiquidity;
    }
    ...
    function setPriceValidationParams(
        ...
        require(_priceImpactThreshold > 0 && _priceImpactThreshold <= 5000, "Invalid price impact threshold");
        require(_maxPriceChangePerUpdate > 0 && _maxPriceChangePerUpdate <= 5000, "Invalid max price change");
        
        priceValidation.priceImpactThreshold = _priceImpactThreshold;
        priceValidation.maxPriceChangePerUpdate = _maxPriceChangePerUpdate;
    }

    function forceUpdatePrice(uint256 _newPrice) external onlyRole(GOVERNANCE_ROLE) {
        require(_newPrice > 0, "Invalid price");
        
        // Even forced updates should have some bounds for new token
        if (newTokenMode && qorafiPriceTwap > 0) {
            uint256 maxChange = Math.mulDiv(qorafiPriceTwap, 5000, 10000); // 50% max change even for forced update
            require(_newPrice >= qorafiPriceTwap - maxChange && _newPrice <= qorafiPriceTwap + maxChange, "Forced price change too large");
        }
    ...
    function invalidateObservation(uint256 index, string calldata reason) external onlyRole(GOVERNANCE_ROLE) {
        require(index < twapObservations.length, "Index out of bounds");
        if (twapObservations[index].isValid) {
            validObservationCount--;
        }
        twapObservations[index].isValid = false;
    ...
    function getLiquidityStatus() external view returns (
        ...
        uint256 current = _getCurrentLiquidityUSDT();
        return (
            current,
            minimumUsdtLiquidity,
            lastLiquidityCheck,
            current >= minimumUsdtLiquidity
        );
    }
