...
/**
 * @title QoraFiGovernor (Hardened Version)
 ...
 */
contract QoraFiGovernor is Governor, GovernorSettings, GovernorCountingSimple, GovernorVotes, GovernorTimelockControl, ReentrancyGuard {
        ...
        GovernorTimelockControl(_timelock)
    {
        ...
        // CRITICAL FIX: Grant the main governance role to the Timelock contract, not the deployer.
        _grantRole(GOVERNANCE_ROLE, address(_timelock));
        
        // Grant the snapshot role to the deployer for initial setup. This should be revoked later.
     ...
     * @dev Should be called periodically by a keeper or DAO to maintain accurate historical voting power.
     */
    function createLpSnapshot() external nonReentrant onlyRole(SNAPSHOT_ROLE) {
        ...
        if (snapshots.length >= MAX_SNAPSHOTS) {
            // Gas-efficiently shift the array left to prune the oldest snapshot
    ...
    /**
     * @dev Overrides the vote counting mechanism to include staked LP tokens, using historical snapshots for security.
     */
    ...
    /**
     * @dev Finds the most recent snapshot at or before a given block number using a binary search.
     */
    ...
    /**
     * @notice Allows governance to grant the SNAPSHOT_ROLE to a keeper or other authorized contract.
     */
    ...
    /**
     * @notice Allows governance to revoke the SNAPSHOT_ROLE.
     */
    function revokeSnapshotRole(address account) external onlyRole(GOVERNANCE_ROLE) {
        if (account == address(0)) revert InvalidAddress();
        revokeRole(SNAPSHOT_ROLE, account);
    ...
    {
        walletVotes = super._getVotes(account, blockNumber, "");
    ...
    function votingDelay()
        public view override(IGovernor, GovernorSettings) returns (uint256) { return super.votingDelay(); }
    function votingPeriod()
        public view override(IGovernor, GovernorSettings) returns (uint256) { return super.votingPeriod(); }
    function state(uint256 proposalId)
        public view override(IGovernor, GovernorTimelockControl) returns (ProposalState) { return super.state(proposalId); }
